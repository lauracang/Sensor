...
com = CreateSerialConnection('COM3',512000);

InputBufferSize=1000;
total_samples=1000000; % the script reads this many samples and then stops.
...
while(k<total_samples)
   % disp('in loop for collection')
    ...
    timestamp(k)=t;
    timenow=datestr(now, 'HH:MM:SS.FFF');
    %disp(timestamp(k))
    ...
    dlmwrite('eeg2-pilot1data.csv', data, '-append');
    %dlmwrite('eeg2-pilot1time.csv', timestamp, '-append');
    dlmwrite('eeg2-pilot1timenow.csv', timenow, '-append');
